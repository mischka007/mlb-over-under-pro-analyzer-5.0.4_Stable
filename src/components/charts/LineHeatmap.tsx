    ...
    <Card accent="gold">
      ...
      <div className="grid grid-cols-9 gap-1">
        {offsets.map((offset) => {
          ...
          const { overProbability } = computePoissonModel(expectedRuns, candidateLine);
          const intensity = Math.round(overProbability * 100);
          return (
            <div
              key={offset}
              className="rounded-md py-2 text-center border border-base-600"
              style={{ background: `rgba(234,179,8,${overProbability * 0.55 + 0.05})` }}
            >
      ...
      <div className="font-mono text-[10px] text-slate-500 mt-2">
        Zelle zeigt Über-Wahrscheinlichkeit bei λ = {expectedRuns.toFixed(2)} für die jeweilige Linie. Aktuelle Linie: {formatPercent(computePoissonModel(expectedRuns, line).overProbability)}
      </div>
    </Card>
  );
}
