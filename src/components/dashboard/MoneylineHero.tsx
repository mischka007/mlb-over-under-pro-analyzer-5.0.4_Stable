...
import type { MoneylineAnalysis, QualityAssessment } from "@/types";
import { Badge, MetricTile } from "@/components/common/UI";
import { formatCurrency, formatPercent, formatSigned } from "@/utils/format";

/**
 * Version 7.1 — Moneyline Premium-Karte. Nutzt exakt dieselbe Struktur
 * wie `PredictionHero.tsx`/`RunLineHero.tsx` (`MetricTile` aus
 * `@/components/common/UI`, identische Typografie/Animation) — keine
 * dritte, abweichende Kartenimplementierung.
 */
export function MoneylineHero({ analysis, quality }: { analysis: MoneylineAnalysis; quality?: QualityAssessment }) {
  ...
  return (
    ...
    >
      <div className="font-mono text-[11px] uppercase tracking-[0.2em] text-slate-500 mb-2">Premium Prediction · Moneyline</div>

      <div className={`flex items-center justify-center gap-3 font-display text-5xl sm:text-6xl font-bold uppercase ${pickColor}`}>
        <Trophy size={40} />
        {teamLabel} ML
      </div>
      <div className="font-numeric text-6xl sm:text-7xl text-slate-100 leading-none mt-2">{formatPercent(recommendation.probability, 1)}</div>
      ...
      <div className="mt-2 flex items-center justify-center gap-2 flex-wrap">
        ...
        {quality && (
          <>
            ...
            <Badge tone={quality.tier === "Premium Bet" ? "gold" : "neutral"}>{quality.tier}</Badge>
          </>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
        {analysis.outcomes.map((outcome) => (
          <div key={outcome.team} className="rounded-lg border border-base-600 bg-base-800/60 px-3 py-2.5 text-left">
            <div className="font-mono text-[9px] uppercase tracking-wider text-slate-500 mb-1">{outcome.team === "home" ? "Heim" : "Auswärts"} ML</div>
            <div className="flex items-center justify-between font-mono text-[11px] text-slate-300">
              <span>{(outcome.winProbability * 100).toFixed(1)}%</span>
              <span className="text-slate-100">Fair {outcome.fairOdds.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
